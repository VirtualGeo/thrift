...
void t_cpp_generator::init_generator() {
  ...
  // Start ifndef
  f_types_ << "#ifndef " << program_name_ << "_TYPES_H" << endl
  ...
  f_types_tcc_ << "#ifndef " << program_name_ << "_TYPES_TCC" << endl
...
void t_cpp_generator::generate_enum(t_enum* tenum) {
  ...
  if (!gen_pure_enums_) {
    enum_name = "type";
    f_types_ << indent() << "struct THRIFT_DLLEXPORT " << tenum->get_name() << " {" << endl;
    indent_up();
  }
...
void t_cpp_generator::generate_enum_ostream_operator(std::ostream& out, t_enum* tenum) {
  ...
  if (!has_custom_ostream(tenum)) {
    ...
    out << indent() << "std::map<int, const char*>::const_iterator it = _" << tenum->get_name()
        << "_VALUES_TO_NAMES.find(val);" << endl;
...
void t_cpp_generator::generate_consts(std::vector<t_const*> consts) {
  ...
  f_consts << "class THRIFT_DLLEXPORT " << program_name_ << "Constants {" << endl
           << " public:" << endl
  ...
  indent_up();
  for (c_iter = consts.begin(); c_iter != consts.end(); ++c_iter) {
    print_const_value(f_consts_impl, (*c_iter)->get_name(), (*c_iter)->get_type(),
                      (*c_iter)->get_value());
  }
  indent_down();
...
void t_cpp_generator::generate_struct_declaration(ostream& out,
  ...
  if (has_nonrequired_fields && (!pointers || read)) {
    ...
    out << indent() << "typedef struct THRIFT_DLLEXPORT _" << tstruct->get_name() << "__isset {"
        << endl;
    indent_up();
  ...
  // Open struct def
  out << indent() << "class THRIFT_DLLEXPORT " << tstruct->get_name() << extends << " {" << endl
      << indent() << " public:" << endl
      << endl;
  indent_up();
  ...
  if (has_required_fields) {
    ...
    for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
      if ((*m_iter)->get_req() == t_field::T_REQUIRED) {
        ...
        out << type_name((*m_iter)->get_type()) << " _" << (*m_iter)->get_name();
  ...
  // Declare all fields
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    indent(out) << declare_field(*m_iter, false,
                                 (pointers && !(*m_iter)->get_type()->is_xception()), !read)
                << endl;
  }
...
void t_cpp_generator::generate_struct_reader(ostream& out, t_struct* tstruct, bool pointers) {
  ...
  out << indent() << "if (ftype == ::apache::thrift::protocol::T_STOP) {" << endl
      << indent() << "  break;" << endl
      << indent() << "}" << endl;
...
void t_cpp_generator::generate_struct_writer(ostream& out, t_struct* tstruct, bool pointers) {
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    bool check_if_set
        = (*f_iter)->get_req() == t_field::T_OPTIONAL || (*f_iter)->get_type()->is_xception();
    if (check_if_set) {
  ...
  out << indent() << "xfer += oprot->writeFieldStop();" << endl
...
void generate_fields(std::ostream& out, const vector<t_field*>& fields, const std::string& indent) {
...
void t_cpp_generator::generate_service(t_service* tservice) {
  ...
  if (gen_templates_) {
    ...
    f_service_tcc_ << "#ifndef " << svcname << "_TCC" << endl
...
void t_cpp_generator::generate_service_interface(t_service* tservice, string style) {
  ...
  f_header_ << "class THRIFT_DLLEXPORT " << service_if_name << extends << " {" << endl
            << " public:" << endl;
  indent_up();
...
void t_cpp_generator::generate_service_interface_factory(t_service* tservice, string style) {
  ...
  f_header_ << "class THRIFT_DLLEXPORT " << factory_name << extends << " {" << endl
            << " public:" << endl;
  indent_up();
  f_header_ << indent() << "typedef " << service_if_name << " Handler;" << endl
            << endl
            << indent() << "virtual ~" << factory_name << "() {}" << endl
            << endl
            << indent() << "virtual " << service_if_name << "* getHandler("
            << "const ::apache::thrift::TConnectionInfo& connInfo) = 0;" << endl
            << indent() << "virtual void releaseHandler(" << base_if_name << "* /* handler */) = 0;"
            << endl;
  ...
  f_header_ << "class THRIFT_DLLEXPORT " << singleton_factory_name << " : virtual public "
            << factory_name << " {" << endl
            << " public:" << endl;
  indent_up();
  f_header_ << indent() << singleton_factory_name << "(const ::apache::thrift::stdcxx::shared_ptr<"
            ...
            << indent() << "}" << endl
            << indent() << "virtual void releaseHandler(" << base_if_name << "* /* handler */) {}"
            << endl;

  f_header_ << endl
            << " protected:" << endl
            << indent() << "::apache::thrift::stdcxx::shared_ptr<" << service_if_name << "> iface_;"
            << endl;
...
void t_cpp_generator::generate_service_null(t_service* tservice, string style) {
  ...
  f_header_ << "class THRIFT_DLLEXPORT " << service_name_ << style << "Null : virtual public "
            << service_name_ << style << "If" << extends << " {" << endl
            << " public:" << endl;
  indent_up();
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (style == "") {
      if (returntype->is_void() || is_complex_type(returntype)) {
        ...
        f_header_ << indent() << declare_field(&returnfield, true) << endl
...
void t_cpp_generator::generate_service_async_skeleton(t_service* tservice) {
  ...
  f_skeleton << "class THRIFT_DLLEXPORT " << svcname << "AsyncHandler : "
             << "public " << svcname << "CobSvIf {" << endl
             << " public:" << endl;
  indent_up();
  f_skeleton << indent() << svcname << "AsyncHandler() {" << endl
             << indent() << "  syncHandler_ = std::auto_ptr<" << svcname << "Handler>(new "
             << svcname << "Handler);" << endl
...
void t_cpp_generator::generate_service_multiface(t_service* tservice) {
  ...
  string list_type
      = string("std::vector<apache::thrift::stdcxx::shared_ptr<") + service_name_ + "If> >";

  // Generate the header portion
  f_header_ << "class THRIFT_DLLEXPORT " << service_name_ << "Multiface : "
  ...
  if (!extends.empty()) {
    f_header_ << indent()
              << "  std::vector<apache::thrift::stdcxx::shared_ptr<" + service_name_
                     + "If> >::iterator iter;"
              << endl
  ...
  indent_up();
  f_header_ << indent() << list_type << " ifaces_;" << endl
            << indent() << service_name_ << "Multiface() {}" << endl
            << indent() << "void add(::apache::thrift::stdcxx::shared_ptr<" << service_name_
            << "If> iface) {" << endl;
  if (!extends.empty()) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    indent_up();
    f_header_ << indent() << "size_t sz = ifaces_.size();" << endl
...
void t_cpp_generator::generate_service_client(t_service* tservice, string style) {
  ...
  f_header_ << template_header << "class THRIFT_DLLEXPORT " << service_name_ << style << "Client"
            << short_suffix << " : "
  ...
  if (style != "Cob") {
    ...
    if (extends.empty()) {
      f_header_ << "{" << endl;
      f_header_ << indent() << "  setProtocol" << short_suffix << "(prot);" << endl
                << indent() << "}" << endl;
    } else {
      f_header_ << ":" << endl;
    ...
    if (extends.empty()) {
      ...
      f_header_ << indent() << "  piprot_=iprot;" << endl
    ...
    if (extends.empty()) {
      f_header_ << indent() << "  channel_(channel)," << endl
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (style == "Concurrent" && !(*f_iter)->is_oneway()) {
      ...
      t_function send_function(g_type_void, string("send_") + (*f_iter)->get_name(),
                               (*f_iter)->get_arglist());
      indent(f_header_) << function_signature(&send_function, "") << ";" << endl;
    }
    if (!(*f_iter)->is_oneway()) {
      if (style == "Concurrent") {
        ...
        t_function recv_function((*f_iter)->get_returntype(),
                                 string("recv_") + (*f_iter)->get_name(), &seqIdArgStruct);
  ...
  if (extends.empty()) {
    ...
    if (style == "Cob") {
      f_header_ << indent()
                ...
                << indent()
                << "::apache::thrift::stdcxx::shared_ptr< "
                   "::apache::thrift::transport::TMemoryBuffer> itrans_;"
                << endl
                << indent()
                << "::apache::thrift::stdcxx::shared_ptr< "
    ...
    f_header_ << indent() << prot_ptr << " piprot_;" << endl
              << indent() << prot_ptr << " poprot_;" << endl
              << indent() << protocol_type << "* iprot_;" << endl
              << indent() << protocol_type << "* oprot_;" << endl;

    if (style == "Concurrent") {
      f_header_ << indent() << "::apache::thrift::async::TConcurrentClientSyncInfo sync_;" << endl;
    }
    indent_down();
  }
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (style != "Cob") {
      ...
      if (!(*f_iter)->is_oneway()) {
        out << indent() << _this << "channel_->sendAndRecvMessage("
            << "::apache::thrift::stdcxx::bind(cob, this), " << _this << "otrans_.get(), " << _this
            << "itrans_.get());" << endl;
      } else {
    ...
    if (true) {
      ...
      t_function send_function(send_func_return_type, string("send_") + (*f_iter)->get_name(),
                               (*f_iter)->get_arglist());
      ...
      out << indent() << "int32_t cseqid = " << cseqidVal << ";" << endl;
      if (style == "Concurrent") {
        out << indent() << "::apache::thrift::async::TConcurrentSendSentry sentry(&this->sync_);"
            << endl;
      }
      if (style == "Cob") {
        out << indent() << _this << "otrans_->resetBuffer();" << endl;
      }
      out << indent() << _this << "oprot_->writeMessageBegin(\"" << (*f_iter)->get_name()
      ...
      out << indent() << "args.write(" << _this << "oprot_);" << endl
          << endl
      ...
      if (!(*f_iter)->is_oneway()) {
        ...
        t_function recv_function((*f_iter)->get_returntype(),
                                 string("recv_") + (*f_iter)->get_name(), recv_function_args);
        ...
        if (style == "Concurrent") {
          out << endl
              << indent()
              << "// the read mutex gets dropped and reacquired as part of waitForWork()" << endl
        ...
        out << indent() << _this << "iprot_->readMessageBegin(fname, mtype, rseqid);" << endl;
        if (style == "Concurrent") {
          scope_down(out);
          out << indent() << "if(seqid == rseqid) {" << endl;
          indent_up();
        }
        out << indent() << "if (mtype == ::apache::thrift::protocol::T_EXCEPTION) {" << endl
        ...
        if (style == "Cob" && !gen_no_client_completion_) {
          out << indent() << "  completed = true;" << endl
        ...
        if (!(*f_iter)->get_returntype()->is_void()) {
          if (is_complex_type((*f_iter)->get_returntype())) {
            ...
            out << indent() << "  // _return pointer has now been filled" << endl;
            if (style == "Cob" && !gen_no_client_completion_) {
              out << indent() << "  completed = true;" << endl
                  << indent() << "  completed__(true);" << endl;
            }
        ...
        for (x_iter = xceptions.begin(); x_iter != xceptions.end(); ++x_iter) {
          ...
          out << indent() << "  throw result." << (*x_iter)->get_name() << ";" << endl
              << indent() << "}" << endl;
        }
...
void ProcessorGenerator::generate_class_definition() {
  ...
  // Generate the header portion
  f_header_ << template_header_ << "class THRIFT_DLLEXPORT " << class_name_ << " : public "
            << parent_class << " {" << endl;

  // Protected data members
  f_header_ << " protected:" << endl;
  indent_up();
  f_header_ << indent() << "::apache::thrift::stdcxx::shared_ptr<" << if_name_ << "> iface_;"
            << endl;
  ...
  if (generator_->gen_templates_) {
    f_header_ << indent() << "typedef void (" << class_name_ << "::*"
              ...
              << "Protocol_*, Protocol_*" << call_context_decl_ << ");" << endl
  ...
  f_header_ << " public:" << endl
            << indent() << class_name_ << "(::apache::thrift::stdcxx::shared_ptr<" << if_name_
            << "> iface) :" << endl;
  if (!extends_.empty()) {
  ...
  if (generator_->gen_templates_) {
    ...
    f_header_ << "typedef " << class_name_ << "< ::apache::thrift::protocol::TDummyProtocol > "
              << service_name_ << pstyle_ << "Processor;" << endl
              << endl;
  }
}
...
void ProcessorGenerator::generate_dispatch_call(bool template_protocol) {
  ...
  f_out_ << indent() << typename_str_ << "ProcessMap::iterator pfn;" << endl
         << indent() << "pfn = processMap_.find(fname);" << endl
         << indent() << "if (pfn == processMap_.end()) {" << endl;
  if (extends_.empty()) {
    f_out_ << indent() << "  iprot->skip(::apache::thrift::protocol::T_STRUCT);" << endl
...
void ProcessorGenerator::generate_factory() {
  ...
  f_header_ << template_header_ << "class THRIFT_DLLEXPORT " << factory_class_name_
  ...
  f_out_ << indent() << "::apache::thrift::ReleaseHandler< " << if_factory_name
         ...
         << indent() << "::apache::thrift::stdcxx::shared_ptr< " << if_name_ << " > handler("
         << "handlerFactory_->getHandler(connInfo), cleanup);" << endl
         << indent() << "::apache::thrift::stdcxx::shared_ptr< ::apache::thrift::"
         << (style_ == "Cob" ? "async::TAsyncProcessor" : "TProcessor") << " > "
         << "processor(new " << class_name_ << template_suffix_ << "(handler));" << endl
...
void t_cpp_generator::generate_process_function(t_service* tservice,
  ...
  if (style != "Cob") {
    ...
    out << indent() << "void* ctx = NULL;" << endl
        << indent() << "if (this->eventHandler_.get() != NULL) {" << endl
        << indent() << "  ctx = this->eventHandler_->getContext(" << service_func_name
        << ", callContext);" << endl
        << indent() << "}" << endl
        << indent() << "::apache::thrift::TProcessorContextFreer freer("
        << "this->eventHandler_.get(), ctx, " << service_func_name << ");" << endl
        << endl
        << indent() << "if (this->eventHandler_.get() != NULL) {" << endl
        << indent() << "  this->eventHandler_->preRead(ctx, " << service_func_name << ");" << endl
        << indent() << "}" << endl
        << endl
        << indent() << argsname << " args;" << endl
    ...
    indent_up();
    out << indent() << "if (this->eventHandler_.get() != NULL) {" << endl
    ...
    out << indent() << "if (this->eventHandler_.get() != NULL) {" << endl
        << indent() << "  this->eventHandler_->preWrite(ctx, " << service_func_name << ");" << endl
        << indent() << "}" << endl
        << endl
        << indent() << "oprot->writeMessageBegin(\"" << tfunction->get_name()
  ...
  // Cob style.
    ...
    out << "void " << tservice->get_name() << "AsyncProcessor" << class_suffix << "::process_"
        << tfunction->get_name()
        << "(::apache::thrift::stdcxx::function<void(bool ok)> cob, int32_t seqid, " << prot_type
        << "* iprot, " << prot_type << "* oprot)" << endl;
    scope_up(out);
    ...
    if (gen_templates_ && !specialized) {
      ...
      out << indent() << "Protocol_* _iprot = dynamic_cast<Protocol_*>(iprot);" << endl
          << indent() << "Protocol_* _oprot = dynamic_cast<Protocol_*>(oprot);" << endl
    ...
    out << indent() << tservice->get_name() + "_" + tfunction->get_name() << "_args args;" << endl
        ...
        << indent() << "if (this->eventHandler_.get() != NULL) {" << endl
        << indent() << "  ctx = this->eventHandler_->getContext(" << service_func_name << ", NULL);"
        << endl
        << indent() << "}" << endl
        << indent() << "::apache::thrift::TProcessorContextFreer freer("
        << "this->eventHandler_.get(), ctx, " << service_func_name << ");" << endl
    ...
    if (tfunction->is_oneway()) {
      ...
      indent_up();
      indent_up();
      out << indent() << "::apache::thrift::stdcxx::bind(return_fn, this, cob, seqid, oprot, ctx"
          << ret_placeholder << ")";
      if (!xceptions.empty()) {
        out << ',' << endl
    ...
    if (!tfunction->is_oneway()) {
      ...
      out << "void " << tservice->get_name() << "AsyncProcessor" << class_suffix << "::return_"
          << tfunction->get_name()
          << "(::apache::thrift::stdcxx::function<void(bool ok)> cob, int32_t seqid, " << prot_type
          << "* oprot, void* ctx" << ret_arg_decl << ')' << endl;
      scope_up(out);
      ...
      if (gen_templates_ && !specialized) {
        ...
        out << indent() << "Protocol_* _oprot = dynamic_cast<Protocol_*>(oprot);" << endl
            ...
            << indent() << "  return return_" << tfunction->get_name() << "(cob, seqid, _oprot, ctx"
            << ret_arg_name << ");" << endl
            << indent() << "}" << endl
            << indent() << "T_GENERIC_PROTOCOL(this, oprot, _oprot);" << endl
            << endl;
      ...
      out << endl
          ...
          << indent() << "oprot->getTransport()->flush();" << endl
          << indent() << "if (this->eventHandler_.get() != NULL) {" << endl
          << indent() << "  this->eventHandler_->postWrite(ctx, " << service_func_name
          << ", bytes);" << endl
          << indent() << "}" << endl
    ...
    if (!tfunction->is_oneway() && !xceptions.empty()) {
      ...
      out << indent() << tservice->get_name() << "_" << tfunction->get_name() << "_result result;"
          ...
          << indent() << "try {" << endl;
      indent_up();
      out << indent() << "_throw->throw_it();" << endl
...
void t_cpp_generator::generate_service_skeleton(t_service* tservice) {
  ...
  f_skeleton << "class THRIFT_DLLEXPORT " << svcname << "Handler : virtual public " << svcname
             << "If {" << endl
             << " public:" << endl;
  indent_up();
  f_skeleton << indent() << svcname << "Handler() {" << endl
             << indent() << "  // Your initialization goes here" << endl
             << indent() << "}" << endl
             << endl;
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    f_skeleton << indent() << function_signature(*f_iter, "") << " {" << endl
  ...
  f_skeleton
      ...
      << indent() << "::apache::thrift::stdcxx::shared_ptr<" << svcname << "Handler> handler(new "
      << svcname << "Handler());" << endl
      << indent() << "::apache::thrift::stdcxx::shared_ptr<TProcessor> processor(new " << svcname
      << "Processor(handler));" << endl
      << indent()
      << "::apache::thrift::stdcxx::shared_ptr<TServerTransport> serverTransport(new "
         "TServerSocket(port));"
      << endl
      << indent()
      << "::apache::thrift::stdcxx::shared_ptr<TTransportFactory> transportFactory(new "
         "TBufferedTransportFactory());"
      << endl
      << indent()
      << "::apache::thrift::stdcxx::shared_ptr<TProtocolFactory> protocolFactory(new "
...
void t_cpp_generator::generate_deserialize_struct(ostream& out,
  ...
  if (pointer) {
    ...
    indent(out) << "  " << prefix << " = ::apache::thrift::stdcxx::shared_ptr<"
                << type_name(tstruct) << ">(new " << type_name(tstruct) << ");" << endl;
    indent(out) << "}" << endl;
...
void t_cpp_generator::generate_deserialize_container(ostream& out, t_type* ttype, string prefix) {
  ...
  if (ttype->is_map()) {
    out << indent() << "::apache::thrift::protocol::TType " << ktype << ";" << endl
        << indent() << "::apache::thrift::protocol::TType " << vtype << ";" << endl
        << indent() << "xfer += iprot->readMapBegin(" << ktype << ", " << vtype << ", " << size
        << ");" << endl;
  } else if (ttype->is_set()) {
    out << indent() << "::apache::thrift::protocol::TType " << etype << ";" << endl
        << indent() << "xfer += iprot->readSetBegin(" << etype << ", " << size << ");" << endl;
  } else if (ttype->is_list()) {
    out << indent() << "::apache::thrift::protocol::TType " << etype << ";" << endl
        << indent() << "xfer += iprot->readListBegin(" << etype << ", " << size << ");" << endl;
    if (!use_push) {
...
void t_cpp_generator::generate_serialize_container(ostream& out, t_type* ttype, string prefix) {
  ...
  out << indent() << type_name(ttype) << "::const_iterator " << iter << ";" << endl
      << indent() << "for (" << iter << " = " << prefix << ".begin(); " << iter << " != " << prefix
      << ".end(); ++" << iter << ")" << endl;
  scope_up(out);
  if (ttype->is_map()) {
...
string t_cpp_generator::function_signature(t_function* tfunction,
  ...
  if (style == "") {
    ...
    return "void " + prefix + tfunction->get_name() + "(::apache::thrift::stdcxx::function<void"
           + cob_type + "> cob" + exn_cob + argument_list(arglist, name_params, true) + ")";
  } else {
    throw "UNKNOWN STYLE";
  }
}
